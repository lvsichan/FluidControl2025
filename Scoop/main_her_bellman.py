...
if __name__ == "__main__":
    ...
    if args.policy == "bellman_SD3_her":
        env_beta_map = {
            'Ant-v2': 0.001,
            'BipedalWalker': 0.05,
            'HalfCheetah': 0.005,
    ...
    replay_buffer_her = her_bellman.ReplayBuffer(state_dim, action_dim, device,max_size=int(args.max_size))
    ...
    state_list_2, action_list_2, next_state_list_2, reward_list_2, done_bool_list_2,isIn_list_2 = [],[],[],[],[],[]
    for t in range(int(args.steps)):
        ...
        if episode_timesteps>1 and not done_bool_last:
            # print("add data")
            replay_buffer_her.add(state_last, action_last, next_state_last, reward_last, done_bool_last,state, action, next_state, reward, done_bool)
            state_list.append(state_last)
            action_list.append(action_last)
            next_state_list.append(next_state_last)
            reward_list.append(reward_last)
            done_bool_list.append(done_bool_last)
            isIn_list.append(isIn_last)
            state_list_2.append(state)
            action_list_2.append(action)
            next_state_list_2.append(next_state)
            reward_list_2.append(reward)
            done_bool_list_2.append(done_bool)
            isIn_list_2.append(isIn)
        if done_bool:
            replay_buffer_her.add(state, action, next_state, reward, done_bool,state, action, next_state, -np.inf, done_bool)
            state_list.append(state_last)
            action_list.append(action_last)
            next_state_list.append(next_state_last)
            reward_list.append(reward_last)
        ...
        if done: 
            # her signal
            if her_flag:
                replay_buffer_her.store_eposide(state_list, action_list, next_state_list, reward_list, done_bool_list,isIn_list,state_list_2, action_list_2, next_state_list_2, reward_list_2, done_bool_list_2,isIn_list_2)
                print("Complete HER!")
            state_list, action_list, next_state_list, reward_list, done_bool_list,isIn_list = [],[],[],[],[],[]
            state_list_2, action_list_2, next_state_list_2, reward_list_2, done_bool_list_2,isIn_list_2 = [],[],[],[],[],[]
