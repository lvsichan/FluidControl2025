...
class SD3(object):
    ...
    def train(self, replay_buffer,replay_buffer_guide, batch_size=100):
        self.train_one_q_and_pi(replay_buffer,replay_buffer_guide, update_q1=True, batch_size=batch_size)
        self.train_one_q_and_pi(replay_buffer,replay_buffer_guide, update_q1=False, batch_size=batch_size)
    ...
    def train_one_q_and_pi(self, replay_buffer,replay_buffer_guide, update_q1, batch_size=100):
        # common_num = int(batch_size*0.5)
        # guide_num = batch_size - common_num
        
        state, action, next_state, reward, not_done, state_2, action_2, next_state_2, reward_2,reward_3, not_done_2 = replay_buffer.sample(batch_size)
        state_g, action_g, next_state_g, reward_g, not_done_g, state_2_g, action_2_g, next_state_2_g, reward_2_g,reward_3_g, not_done_2_g = replay_buffer_guide.sample(batch_size)
        ## stack low and high
        state = torch.cat((state,state_g),dim=0)
        action =  torch.cat((action,action_g),dim=0)
        next_state =  torch.cat((next_state,next_state_g),dim=0)
        reward =  torch.cat((reward,reward_g),dim=0)
        not_done =  torch.cat((not_done,not_done_g),dim=0)
        
        state_2 = torch.cat((state_2,state_2_g),dim=0)
        action_2 =  torch.cat((action_2,action_2_g),dim=0)
        next_state_2 =  torch.cat((next_state_2,next_state_2_g),dim=0)
        reward_2 =  torch.cat((reward_2,reward_2_g),dim=0)
        reward_3 =  torch.cat((reward_3,reward_3_g),dim=0)
        not_done_2 =  torch.cat((not_done_2,not_done_2_g),dim=0)

        with torch.no_grad():
            if update_q1:
                next_action = self.actor1_target(next_state)
                next_action_2 = self.actor1_target(next_state_2)
            else:
            ...
            ######
            next_action_2 = torch.unsqueeze(next_action_2, 1)

            next_action_2 = (next_action_2 + noise).clamp(-self.max_action, self.max_action)

            next_state_2 = torch.unsqueeze(next_state_2, 1)
            next_state_2 = next_state_2.repeat((1, self.num_noise_samples, 1))

            next_Q1_2 = self.critic1_target(next_state_2, next_action_2)
            next_Q2_2 = self.critic2_target(next_state_2, next_action_2)
            ###### UP BOUDARY
            # next_Q1 =  torch.max(next_Q1, reward_2 + not_done_2 * self.discount *next_Q1_2)
            # next_Q2 =  torch.max(next_Q2, reward_2 + not_done_2 * self.discount *next_Q2_2)
            ######
            ...
            next_Q_2 = torch.min(next_Q1_2, next_Q2_2)
            next_Q_2 = torch.squeeze(next_Q_2, 2)
            

            softmax_next_Q_2 = self.softmax_operator(next_Q_2, noise_pdf)
            # next_Q = softmax_next_Q 
            
            next_Q = torch.max(softmax_next_Q, reward_2 + not_done_2 * self.discount *softmax_next_Q_2)
            target_Q = reward + not_done * self.discount * next_Q
            
            target_Q_2 = reward_3 + not_done_2 * self.discount *softmax_next_Q_2
            
            # stack t and t+1
            target_Q = torch.cat((target_Q,target_Q_2), dim=0)
            state = torch.cat((state,state_2), dim=0)
            action = torch.cat((action,action_2), dim=0)
