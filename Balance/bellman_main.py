...
if __name__ == "__main__":
    ...
    writer = SummaryWriter('runs_asia_v2/'+ f"{args.env}_{args.policy}_{args.seed}_{args.beta}_{args.noise_type}_"+now.strftime("%Y-%m-%d_%H-%M-%S")) 
    ...
    replay_buffer = bellman_utils.ReplayBuffer(state_dim, action_dim,max_size=int(args.max_size))
    ...
    for t in range(int(args.steps)):
        ...
        if episode_timesteps>1 and not done_bool_last:
            # print("add data")
            replay_buffer.add(state_last, action_last, next_state_last, reward_last, done_bool_last,state, action, next_state, reward, reward,done_bool)
        if done:
            replay_buffer.add(state, action, next_state, reward, done_bool,state, action, next_state, -np.inf,reward, done_bool)
        state_last = state
        action_last = action
